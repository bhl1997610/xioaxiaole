...
var MapControl = /** @class */ (function () {
    ...
    */
    ...
    MapControl.prototype.createAllMap = function () {
        ...
        for (var i = 0; i < GameData.MaxRow; i++) {
            for (var t = 0; t < GameData.MaxColumn; t++) {
                if (GameData.mapData[i][t] != -1 && GameData.mapData[i][t] != -3) //地图元素可用a
                 {
                    while (haveLink) {
    ...
    MapControl.prototype.changeTypeByID = function (id) {
        ...
        // console.log(id,GameData.elements[id].type + "根据传进来的ID，改变元素的type值");
    };
    /**
    ...
    MapControl.prototype.updateMapLocation = function () {
        //ID 去重
        var ids = new Array(); //当前被消除的所有元素的
        var len = LinkLogic.lines.length; //被消除的元素ID
        for (var i = 0; i < len; i++) {
            var l = LinkLogic.lines[i].length;
            for (var t = 0; t < l; t++) { //循环操作每一个被消除的元素。
        ...
        for (var i = 0; i < len; i++) {
            ...
            for (var t = GameData.MaxRow - 1; t >= 0; t--) //按行 下->上
             {
